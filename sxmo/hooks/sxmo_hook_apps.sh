#!/bin/sh
# configversion: 9cb4b21ffce8870b1fcefdbd15252e15
...
write_line_app anki "$icon_str Anki" "/home/alarm/.local/bin/anki"
...
write_line_app firefox "$icon_str ankiweb.net" "firefox ankiweb.net"
write_line_app firefox "$icon_usr Duolingo" "firefox duolingo.com"
write_line_app firefox "$icon_hom Home Assistant" "firefox https://home.saggyserver.duckdns.org"
write_line_app firefox "$icon_wtr Met Éireann" "firefox https://www.met.ie"
write_line_app spotifyd "$icon_mus spotifyd" "~/.config/sxmo/userscripts/spotifyd.sh"
write_line_app waydroid "$icon_and Waydroid" "~/.config/sxmo/userscripts/waydroid.sh"
write_line_app ytfzf "$icon_ytb ytfzf" 'search=$(echo "" | sxmo_dmenu_with_kb.sh -p Search) && nohup ytfzf -D "$search" >/dev/null 2>&1 &'
write_line_app ytfzf "$icon_ytb ytfzf (Audio Only)" 'search=$(echo "" | sxmo_dmenu_with_kb.sh -p Search) && sxmo_terminal.sh sh -c "ytfzf -D -m \"$search\""'
write_line_app firefox "$icon_msg Element" "firefox https://app.element.io"
write_line_app firefox "$icon_inf Dublin Bus - Real Time Information" "firefox https://www.dublinbus.ie/RTPI"
